...
import { maggie, dublin, testLocations, maggieCredentials } from "../fixtures.js";
...
suite("Location API tests", () => {
  ...
  test("create location", async () => {
    const returnedLocation = await playtimeService.createLocation(dublin);
    assert.isNotNull(returnedLocation);
    assertSubset(dublin, returnedLocation);
  });

  test("delete a location", async () => {
    const location = await playtimeService.createLocation(dublin);
